...
public enum SOCameraState {
    case Ready, AccessDenied, NoDeviceFound, NotDetermined
}

public enum SOCameraDevice {
    case Front, Back
}

public enum SOCameraFlashMode: Int {
    case Off, On, Auto
}

public enum SOCameraOutputMode {
    case StillImage, VideoWithMic, VideoOnly
}

public enum SOCameraOutputQuality: Int {
    case Low, Medium, High
}

/// Class for handling iDevices custom camera usage
public class SOCameraManager: NSObject, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate {
    ...
    public var cameraDevice = SOCameraDevice.Back {
        didSet {
            if cameraIsSetup {
    ...
    public var flashMode = SOCameraFlashMode.Off {
        didSet {
            if cameraIsSetup {
    ...
    public var cameraOutputQuality = SOCameraOutputQuality.High {
        didSet {
            if cameraIsSetup {
    ...
    public var cameraOutputMode = SOCameraOutputMode.StillImage {
        didSet {
            if cameraIsSetup {
    ...
    */
    public func addPreviewLayerToView(view: UIView) -> SOCameraState {
        return addPreviewLayerToView(view, newCameraOutputMode: cameraOutputMode)
    }
    public func addPreviewLayerToView(view: UIView, newCameraOutputMode: SOCameraOutputMode) -> SOCameraState {
        return addPreviewLayerToView(view, newCameraOutputMode: newCameraOutputMode, completition: nil)
    }
    public func addPreviewLayerToView(view: UIView, newCameraOutputMode: SOCameraOutputMode, completition: (Void -> Void)?) -> SOCameraState {
        if _canLoadCamera() {
    ...
    /**
     ...
     :returns: Current state of the camera: Ready / AccessDenied / NoDeviceFound / NotDetermined
     */
    public func currentCameraStatus() -> SOCameraState {
        return _checkIfCameraIsAvailable()
    }
     ...
     */
    public func changeFlashMode() -> SOCameraFlashMode {
        flashMode = SOCameraFlashMode(rawValue: (flashMode.rawValue+1)%3)!
        return flashMode
    }
     ...
     */
    public func changeQualityMode() -> SOCameraOutputQuality {
        cameraOutputQuality = SOCameraOutputQuality(rawValue: (cameraOutputQuality.rawValue+1)%3)!
    ...
    private func _updateTorch(flashMode: SOCameraFlashMode) {
        captureSession?.beginConfiguration()
        let devices = AVCaptureDevice.devicesWithMediaType(AVMediaTypeVideo)
        for  device in devices  {
            let captureDevice = device as! AVCaptureDevice
    ...
    private func _canLoadCamera() -> Bool {
        let currentSOCameraState = _checkIfCameraIsAvailable()
        return currentSOCameraState == .Ready || (currentSOCameraState == .NotDetermined && showAccessPermissionPopupAutomatically)
    }
    ...
    private func _checkIfCameraIsAvailable() -> SOCameraState {
    ...
    private func _setupOutputMode(newCameraOutputMode: SOCameraOutputMode, oldCameraOutputMode: SOCameraOutputMode?) {
        captureSession?.beginConfiguration()
    ...
    private func _updateCameraDevice(deviceType: SOCameraDevice) {
        if let validCaptureSession = captureSession {
            validCaptureSession.beginConfiguration()
    ...
    private func _updateFlasMode(flashMode: SOCameraFlashMode) {
        captureSession?.beginConfiguration()
        let devices = AVCaptureDevice.devicesWithMediaType(AVMediaTypeVideo)
        for  device in devices  {
            let captureDevice = device as! AVCaptureDevice
    ...
    private func _updateCameraQualityMode(newCameraOutputQuality: SOCameraOutputQuality) {
        if let validCaptureSession = captureSession {
            var sessionPreset = AVCaptureSessionPresetLow
            switch (newCameraOutputQuality) {
            case SOCameraOutputQuality.Low:
                sessionPreset = AVCaptureSessionPresetLow
            case SOCameraOutputQuality.Medium:
                sessionPreset = AVCaptureSessionPresetMedium
            case SOCameraOutputQuality.High:
                if cameraOutputMode == .StillImage {
